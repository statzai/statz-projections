...
class ProjectionService:
    ...
    async def projections(self, league_request):
        ...
        # Accuracy Dataset
        projection_accuracy_dataset_league = pd.read_excel(os.path.join(data_folder_path, f"{league}_accuracy_dataset.xlsx"))
        projection_accuracy_dataset_all = pd.read_excel(os.path.join(data_folder_path, "all_leagues_accuracy_dataset.xlsx"))

        # Ratings Dataset
        all_team_ratings = pd.read_excel(os.path.join(data_folder_path, "Team Ratings.xlsx"))
        all_team_ratings['Date'] = all_team_ratings['Date'].dt.date

        # In[9]:
        player_stats = pd.read_csv(os.path.join(data_folder_path, "fixture_player_stats.csv"))
        player_stats.drop_duplicates(subset=['fixture_id', 'player_id', 'stats_type_id'], inplace=True)
        team_stats = pd.read_csv(os.path.join(data_folder_path, "fixture_team_stats.csv"))
        team_stats.drop_duplicates(subset=['fixture_id', 'team_id', 'stats_type_id'], inplace=True)
        standings = pd.read_csv(os.path.join(data_folder_path, "standings.csv"))



        seasons = pd.read_csv(os.path.join(data_folder_path, "seasons.csv"))
        comps = pd.read_csv(os.path.join(data_folder_path, "competitions.csv"))
        comp_teams = pd.read_csv(os.path.join(data_folder_path, "competition_season_teams.csv"))
        teams = pd.read_csv(os.path.join(data_folder_path, "teams.csv"))
        players = pd.read_csv(os.path.join(data_folder_path, "players.csv"))
        players['display_name'] = players['display_name'].str.strip()
        fixtures_df = pd.read_csv(os.path.join(data_folder_path, "fixtures.csv"))
        fixtures_df.drop_duplicates(subset=['season_id', 'home_team_id', 'away_team_id', 'kickoff_datetime'],
                                    inplace=True)  # CHANGED - This now includes kickoff_datetime as teams can play each other more than twice in a season
        b365_odds = pd.read_csv(os.path.join(data_folder_path, "bet365_odds.csv"))  # NEW - This loads in the Bet365 odds data
        b365_odds = b365_odds.drop_duplicates(subset=['fixture_id', 'name'],
                                              keep='last')  # NEW - This removes any duplicate odds entries for the same fixture and market, keeping the last one
        fixtures_df['over_1_5_odds_decimal'] = fixtures_df['id'].map(
            b365_odds[b365_odds['name'] == 'OVER_1_5'].set_index('fixture_id')[
                'odd_decimal'])  # NEW - This maps the over 1.5 goals odds to the fixtures dataframe
        fixtures_df['over_2_5_odds_decimal'] = fixtures_df['id'].map(
            b365_odds[b365_odds['name'] == 'OVER_2_5'].set_index('fixture_id')[
                'odd_decimal'])  # NEW - This maps the over 2.5 goals odds to the fixtures dataframe
        stats_types = pd.read_csv(os.path.join(data_folder_path, "stats_types.csv"))
        ...
        for stat in stat_list:
            ...
            if os.path.exists(file_path_all):
                ...
                print(f"Napravljeno i snimljeno: All_Leagues_{stat}_model.sav")
        ...
        #                                               index=False)
        file_path = os.path.join(data_folder_path, "Accuracy Dataset with Errors.xlsx")
        projection_accuracy_dataset_all_copy.to_excel(file_path, index=False)
        ...
        if len(teams_to_map) > 0:
            print('Statz Team Names to Map:')
            print(teams_to_map.to_string(index=False))
            market_values_not_mapped = market_values[~market_values['Team'].isin(ratings['Team'])]
            print('\nMarket Value Team Names to Map the above names to:')
            print(market_values_not_mapped['Team'].to_string(index=False))
            raise ValueError(
        ...
        all_team_ratings.to_excel(f"{data_folder_path}/Team Ratings.xlsx", index=False)

        print("team ratings from " + league)
        print(all_team_ratings[all_team_ratings['League'] == league])
        ...
        print("avg_away_goals: ", avg_away_goals)
        ...
        best_bets = pd.read_excel(f"{data_folder_path}/Best Bets.xlsx")
        ...
        best_bets.to_excel(f"{data_folder_path}/Best Bets.xlsx", index=False)
        ...
        model_dataset_league.to_excel(f"{data_folder_path}/{league}_model_dataset_with_history.xlsx", index=False)
        model_dataset_all.to_excel(f"{data_folder_path}/all_leagues_model_dataset_with_history.xlsx", index=False)
        ...
        print('team_projections 1059:')
        print(list(team_projections.columns))
        ...
        team_projections_save = team_projections.copy()
        print("team_projections_save: ", team_projections_save)
        ...
        projection_accuracy_dataset_league.to_excel(f"{data_folder_path}/{league}_accuracy_dataset.xlsx", index=False)
        ...
        projection_accuracy_dataset_all.to_excel(f"{data_folder_path}/all_leagues_accuracy_dataset.xlsx", index=False)
        ...
        print("season_id:", [current_season_id, previous_season_id, previous_season_id_above, previous_season_id_below])
        ...
        print('pl_projections: ', pl_projections)
        start = []
    ...
    async def fixtures(self, league_request):
        ...
        # Accuracy Dataset
        projection_accuracy_dataset_league = pd.read_excel(
            os.path.join(data_folder_path, f"{league}_accuracy_dataset.xlsx"))
        projection_accuracy_dataset_all = pd.read_excel(
            os.path.join(data_folder_path, "all_leagues_accuracy_dataset.xlsx"))
        ...
        fixtures_df.drop_duplicates(subset=['season_id', 'home_team_id', 'away_team_id', 'kickoff_datetime'],
                                    inplace=True)  # CHANGED - This now includes kickoff_datetime as teams can play each other more than twice in a season
        b365_odds = pd.read_csv(
            os.path.join(data_folder_path, "bet365_odds.csv"))  # NEW - This loads in the Bet365 odds data
        b365_odds = b365_odds.drop_duplicates(subset=['fixture_id', 'name'],
                                              keep='last')  # NEW - This removes any duplicate odds entries for the same fixture and market, keeping the last one
        fixtures_df['over_1_5_odds_decimal'] = fixtures_df['id'].map(
            b365_odds[b365_odds['name'] == 'OVER_1_5'].set_index('fixture_id')[
        ...
        all_team_ratings[all_team_ratings['League'] == league].to_csv(f"{save_file_path}/{league} Team Ratings.csv",
                                                                      index=False)
    ...
    async def predicted_table(self, league_request):
        ...
        league_dashed = league.replace(' ', '-').replace('.', '').lower()
        file_path = os.path.join(ProjectionService.DATA_FOLDER_PATH, "League Weightings.xlsx")
        league_weightings_df = pd.read_excel(file_path)
        ...
        # Model Dataset
        file_path = os.path.join(ProjectionService.DATA_FOLDER_PATH, "all_leagues_model_dataset_with_history.xlsx")
        model_dataset_all = pd.read_excel(file_path)
        file_path = os.path.join(ProjectionService.DATA_FOLDER_PATH, f"{league}_model_dataset_with_history.xlsx")
        model_dataset_league = pd.read_excel(file_path)

        # Accuracy Dataset
        projection_accuracy_dataset_league = pd.read_excel(
            os.path.join(ProjectionService.DATA_FOLDER_PATH, f"{league}_accuracy_dataset.xlsx"))
        projection_accuracy_dataset_all = pd.read_excel(
            os.path.join(ProjectionService.DATA_FOLDER_PATH, "all_leagues_accuracy_dataset.xlsx"))

        # Ratings Dataset
        all_team_ratings = pd.read_excel(os.path.join(ProjectionService.DATA_FOLDER_PATH, "Team Ratings.xlsx"))
        all_team_ratings['Date'] = all_team_ratings['Date'].dt.date

        # In[9]:
        player_stats = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "fixture_player_stats.csv"))
        player_stats.drop_duplicates(subset=['fixture_id', 'player_id', 'stats_type_id'], inplace=True)
        team_stats = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "fixture_team_stats.csv"))
        team_stats.drop_duplicates(subset=['fixture_id', 'team_id', 'stats_type_id'], inplace=True)
        standings = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "standings.csv"))

        seasons = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "seasons.csv"))
        comps = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "competitions.csv"))
        comp_teams = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "competition_season_teams.csv"))
        teams = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "teams.csv"))
        players = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "players.csv"))
        players['display_name'] = players['display_name'].str.strip()
        fixtures_df = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "fixtures.csv"))
        fixtures_df.drop_duplicates(subset=['season_id', 'home_team_id', 'away_team_id', 'kickoff_datetime'],
                                    inplace=True)  # CHANGED - This now includes kickoff_datetime as teams can play each other more than twice in a season
        b365_odds = pd.read_csv(
            os.path.join(ProjectionService.DATA_FOLDER_PATH, "bet365_odds.csv"))  # NEW - This loads in the Bet365 odds data
        b365_odds = b365_odds.drop_duplicates(subset=['fixture_id', 'name'],
                                              keep='last')  # NEW - This removes any duplicate odds entries for the same fixture and market, keeping the last one
        fixtures_df['over_1_5_odds_decimal'] = fixtures_df['id'].map(
            b365_odds[b365_odds['name'] == 'OVER_1_5'].set_index('fixture_id')[
        ...
        fixtures_df['over_2_5_odds_decimal'] = fixtures_df['id'].map(
            b365_odds[b365_odds['name'] == 'OVER_2_5'].set_index('fixture_id')[
                'odd_decimal'])  # NEW - This maps the over 2.5 goals odds to the fixtures dataframe
        stats_types = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "stats_types.csv"))
        ...
        for stat in stat_list:
            ...
            # Putanja do modela po ligi
            file_path = os.path.join(ProjectionService.MODEL_FILE_PATH, league, f"{league}_{stat}_model.sav")
            ...
            if os.path.exists(file_path):
                ...
                with open(file_path, 'wb') as f:
                    pickle.dump(model, f)
                print(f"Napravljeno i snimljeno: {league}_{stat}_model.sav")

            # Isto za model svih liga
            folder_path = os.path.join(ProjectionService.MODEL_FILE_PATH, "All Leagues")
            os.makedirs(folder_path, exist_ok=True)
            file_path_all = os.path.join(folder_path, f"All_Leagues_{stat}_model.sav")
        ...
        accuracy_df_league = accuracy_df_league.round(2)
        accuracy_df_all = accuracy_df_all.round(2)
        # accuracy_df_league.to_csv(rf"{ProjectionService.DATA_FOLDER_PATH}\{league} Projection Accuracy.csv", index=False)
        # accuracy_df_all.to_csv(rf"{ProjectionService.DATA_FOLDER_PATH}\All Leagues Projection Accuracy.csv", index=False)

        # Za league
        file_path_league = os.path.join(ProjectionService.DATA_FOLDER_PATH, f"{league} Projection Accuracy.csv")
        accuracy_df_league.to_csv(file_path_league, index=False)

        # Za sve lige
        file_path_all = os.path.join(ProjectionService.DATA_FOLDER_PATH, "All Leagues Projection Accuracy.csv")
        accuracy_df_all.to_csv(file_path_all, index=False)
        ...
        # projection_accuracy_dataset_all_copy.to_excel(rf"{ProjectionService.DATA_FOLDER_PATH}\Accuracy Dataset with Errors.xlsx",
        #                                               index=False)
        file_path = os.path.join(ProjectionService.DATA_FOLDER_PATH, "Accuracy Dataset with Errors.xlsx")
        projection_accuracy_dataset_all_copy.to_excel(file_path, index=False)
        ...
        try:
            # second_ratings = pd.read_excel(rf"{ProjectionService.DATA_FOLDER_PATH}\{league} Promoted Team Ratings.xlsx")
            second_ratings = pd.read_excel(f"{ProjectionService.DATA_FOLDER_PATH}/{league} Promoted Team Ratings.xlsx")
            second_ratings = second_ratings[['Team', 'Attack', 'Defense']]
            second_ratings['Attack'] = (second_ratings['Attack']) * league_below_attack_weight
            second_ratings['Defense'] = (second_ratings[
                'Defense']) / league_below_defense_weight  # UPDATED - divide instead of multiply
        ...
        all_team_ratings.drop_duplicates(subset=['Team', 'League', 'Date'], keep='last', inplace=True)
        all_team_ratings.reset_index(drop=True, inplace=True)
        # all_team_ratings.to_excel(rf"{ProjectionService.DATA_FOLDER_PATH}\Team Ratings.xlsx", index=False)
        all_team_ratings.to_excel(f"{ProjectionService.DATA_FOLDER_PATH}/Team Ratings.xlsx", index=False)

        print("team ratings from " + league)
        print(all_team_ratings[all_team_ratings['League'] == league])

        all_team_ratings[all_team_ratings['League'] == league].to_csv(f"{ProjectionService.SAVE_FILE_PATH}/{league} Team Ratings.csv",
                                                                      index=False)
        ...
        best_bets = pd.read_excel(f"{ProjectionService.DATA_FOLDER_PATH}/Best Bets.xlsx")
        ...
        best_bets.to_excel(f"{ProjectionService.DATA_FOLDER_PATH}/Best Bets.xlsx", index=False)
    ...
    async def teams(self, league_request):
        ...
        model_dataset_league.to_excel(f"{ProjectionService.DATA_FOLDER_PATH}/{league}_model_dataset_with_history.xlsx", index=False)
        model_dataset_all.to_excel(f"{ProjectionService.DATA_FOLDER_PATH}/all_leagues_model_dataset_with_history.xlsx", index=False)
    ...
    async def players(self, league_request):
        ...
        # Accuracy Dataset
        projection_accuracy_dataset_league = pd.read_excel(os.path.join(ProjectionService.DATA_FOLDER_PATH, f"{league}_accuracy_dataset.xlsx"))
        projection_accuracy_dataset_all = pd.read_excel(os.path.join(ProjectionService.DATA_FOLDER_PATH, "all_leagues_accuracy_dataset.xlsx"))
        ...
        fixtures_df.drop_duplicates(subset=['season_id', 'home_team_id', 'away_team_id', 'kickoff_datetime'],
                                    inplace=True)  # CHANGED - This now includes kickoff_datetime as teams can play each other more than twice in a season
        b365_odds = pd.read_csv(os.path.join(ProjectionService.DATA_FOLDER_PATH, "bet365_odds.csv"))  # NEW - This loads in the Bet365 odds data
        b365_odds = b365_odds.drop_duplicates(subset=['fixture_id', 'name'],
                                              keep='last')  # NEW - This removes any duplicate odds entries for the same fixture and market, keeping the last one
        fixtures_df['over_1_5_odds_decimal'] = fixtures_df['id'].map(
            b365_odds[b365_odds['name'] == 'OVER_1_5'].set_index('fixture_id')[
        ...
        all_team_ratings[all_team_ratings['League'] == league].to_csv(f"{ProjectionService.SAVE_FILE_PATH}/{league} Team Ratings.csv", index=False)
        ...
        projection_accuracy_dataset_league.to_excel(f"{ProjectionService.DATA_FOLDER_PATH}/{league}_accuracy_dataset.xlsx", index=False)
        ...
        projection_accuracy_dataset_all.to_excel(f"{ProjectionService.DATA_FOLDER_PATH}/all_leagues_accuracy_dataset.xlsx", index=False)
